...
def createObject(adataPath,jsonPath,normalized = False): # TODO : Print normalization process info.
    ...
    adataScores = sc.AnnData(X=scoresArray,var=geneSetNamesArray,obs=adata.obs)

    if normalized == True:

        from sklearn.preprocessing import quantile_transform,StandardScaler

        adataScores.X = quantile_transform(adataScores.X,axis=1,output_distribution="normal")
